...
def game_save(player1_id, player2_id, score1, score2):
    ...
    if (player1.tournament is not None):
        ...
        if (tournament.game1 is None):
            ...
            looser1 = tournament.game1.player1 if tournament.game1.winner == 2 else tournament.game1.player2
            looser2 = tournament.game2.player1 if tournament.game2.winner == 2 else tournament.game2.player2
            ...
            room_name = '_'.join(str(looser1.id))
            room_group_name = 'notification_%s' % room_name
            message = 'You took third-fourth place in the tournament!'
            async_to_sync(channel_layer.group_send)(
                room_group_name,
                {
                    'type': 'tournament_game_notification',
            ...
            room_name = '_'.join(str(looser2.id))
            room_group_name = 'notification_%s' % room_name
            message = 'You took third-fourth place in the tournament!'
            async_to_sync(channel_layer.group_send)(
                room_group_name,
            ...
            winner = tournament.game3.player1 if tournament.game3.winner == 1 else tournament.game3.player2
            looser = tournament.game3.player1 if tournament.game3.winner == 2 else tournament.game3.player2
            tournament.delete()
            channel_layer = get_channel_layer()
            room_name = '_'.join(str(winner.id))
            room_group_name = 'notification_%s' % room_name
            message = 'You have won the tournament!'
            async_to_sync(channel_layer.group_send)(
                room_group_name,
                {
                    'type': 'tournament_game_notification',
            ...
            room_name = '_'.join(str(looser.id))
            room_group_name = 'notification_%s' % room_name
            message = 'You took second place in the tournament!'
            async_to_sync(channel_layer.group_send)(
                room_group_name,
                {
                    'type': 'tournament_game_notification',
            ...
            if self.player.is_authenticated:
                ...
                if count == 1:
                    ...
                    await self.channel_layer.group_send(
                            ...
                            {
                                "type": "game_start",
                                "player1_name": PrivateGameConsumer.players[str(opponent)]["username"],
                                "player2_name": PrivateGameConsumer.players[str(self.player.id)]["username"],
                            },
                        )
        ...
        while len(PrivateGameConsumer.players) > 1:
            ...
            async with self.update_lock:
                ...
                if (PrivateGameConsumer.players[player1Id]["score"] == 3 or PrivateGameConsumer.players[player2Id]["score"] == 3):
                    ...
                    room_name = '_'.join(str(player1Id))
                    room_group_name = 'notification_%s' % room_name
                    result = 'won' if PrivateGameConsumer.players[str(player1Id)]["score"] > PrivateGameConsumer.players[str(player2Id)]["score"] else 'lost'
                    message = 'You %s the game against %s' % (result, PrivateGameConsumer.players[str(player2Id)]["username"])
                    await channel_layer.group_send(
                        room_group_name,
                        {
                    ...
                    message = 'You %s the game against %s' % (result, PrivateGameConsumer.players[str(player1Id)]["username"])
                    await channel_layer.group_send(
                        room_group_name,
                        {
                            'type': 'game_result_notification',
                ...
                if PrivateGameConsumer.players[player1Id]["upPressed"] and PrivateGameConsumer.players[player1Id]["paddleY"] > 0:
                    PrivateGameConsumer.players[player1Id]["paddleY"] -= 15
                ...
                if PrivateGameConsumer.players[player2Id]["upPressed"] and PrivateGameConsumer.players[player2Id]["paddleY"] > 0:
                    PrivateGameConsumer.players[player2Id]["paddleY"] -= 15
            ...
            if self.player.is_authenticated:
                ...
                if (MatchmakingConsumer.waiting_player == False):
                    ...
                    await self.channel_layer.group_send(
                            ...
                            {
                                "type": "game_start",
                                "player1_name": MatchmakingConsumer.players[opponent_index]["username"],
                                "player2_name": MatchmakingConsumer.players[str(self.player.id)]["username"],
                            },
                        )
        ...
        while len(MatchmakingConsumer.players) > 1:
            ...
            async with self.update_lock:
                ...
                if (MatchmakingConsumer.players[player1Id]["score"] == 3 or MatchmakingConsumer.players[player2Id]["score"] == 3):
                    ...
                    message = 'You %s the game against %s' % (result, MatchmakingConsumer.players[player2Id]["username"])
                    await channel_layer.group_send(
                        room_group_name,
                        {
                            'type': 'game_result_notification',
                    ...
                    message = 'You %s the game against %s' % (result, MatchmakingConsumer.players[player1Id]["username"])
                    await channel_layer.group_send(
                        room_group_name,
                        {
                            'type': 'game_result_notification',
                ...
                if MatchmakingConsumer.players[player1Id]["upPressed"] and MatchmakingConsumer.players[player1Id]["paddleY"] > 0:
                    MatchmakingConsumer.players[player1Id]["paddleY"] -= 15
                ...
                if MatchmakingConsumer.players[player2Id]["upPressed"] and MatchmakingConsumer.players[player2Id]["paddleY"] > 0:
                    MatchmakingConsumer.players[player2Id]["paddleY"] -= 15
...
class AIConsumer(AsyncWebsocketConsumer):
    ...
    async def bot_update(self):
        ...
        while True:
            ...
            async with self.update_lock:
                if (self.player is None or self.player["connected"] == False or self.player["score"] == 3 or self.bot["score"] == 3):
                    break
                x = self.x
                y = self.y
                dx = x - previous_x
                ...
                if dx > 0:
                    ...
                    calc_y = y + dy * time_x
                    crash_preventer = 0
                    while (calc_y < self.ball_radius or calc_y > self.canvas_height - self.ball_radius) and crash_preventer < 3:
                        if calc_y < self.ball_radius:
                            calc_y = -calc_y + self.ball_radius
                        elif calc_y > self.canvas_height - self.ball_radius:

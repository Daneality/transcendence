...
const Dashboard = (props) => {
  ...
  const handleProfileUpdate = (event) => {
    ...
    if (!(newProfileData.username || newProfileData.email || newProfileData.password || newProfileData.image)) {
    ...
    for (const key in newProfileData) {
      if (newProfileData[key] !== '') {
        ...
        console.log(newProfileData[key]);
      }
    }
  
    fetch(`https://${window.location.hostname}/api/users/${userData.id}/`, {
      method: 'POST',
      headers: {
      ...
      .catch((error) => {
        ...
        alert('Error: genau das wird ausgelöst', error);
      });
  };

  function fetchUserData() {
    const backendURL = `https://${window.location.hostname}/api/users/`;
  ...
  function addFriend() {
    fetch(`https://${window.location.hostname}/api/friend-requests/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Token ${localStorage.getItem('Token')}`
  ...
  function acceptFriendRequest(requestId) {
    fetch(`https://${window.location.hostname}/api/friend-requests/${requestId}/accept/`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',   
        'Authorization': `Token ${localStorage.getItem('Token')}` 
  ...
  const handleBack = () => {
    ...
    const acceptInvite = (inviteId) => {
      ...
      const backendURL = `https://${window.location.hostname}/api/game-invites/delete/${inviteId.id}/`;
      fetch(backendURL, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
      ...
      <div className="container mt-5">
        ...
        {userData && !isEditingProfile ? (
          ...
          <form method="post">
            ...
            <button className="btn btn-primary mb-2" style={{ height:'30px', backgroundColor: '#000000', color: '#ffffff'}} onClick={(event) => handleProfileUpdate(event)}>update profile</button>

...
const Chat = (props) => {
	...
	const backendURL = `https://${window.location.hostname}/api/chats/`
	...
	useEffect(() => {
		...
		if (selectedFriend) {
		  const newSocket = new WebSocket(`wss://${window.location.hostname}/ws/chat/` + selectedFriend.participant2.id + '/?token=' + localStorage.getItem('Token'));
	...
	const handleInviteClick = (friend) => {
		const backURL = `https://${window.location.hostname}/api/game-invites/create/`;
	...
	const handleBlockUser = (friend) => {
		const backURL = `https://${window.location.hostname}/api/users/` + friend.id + '/block';
		fetch(backURL, {	
			method: 'PUT',
			headers: {
				'Content-Type': 'application/json',
		  ...
		  <button className="btn btn-secondary mb-2" style={{ height:'30px', backgroundColor: '#000000', color: '#ffffff'}}onClick={backtoMenu}>back to profile</button>
			...
			<ListGroup>
				{chatData ? chatData.map((chat, index) => (
					...
					>
					<a href={`/dashboard/${chat.participant2.id}`}>{chat.participant2.username}</a>
